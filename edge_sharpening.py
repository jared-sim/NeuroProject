    ...
    for city in city_list:
        ...
        for item in temp:
            if not item.endswith('leftImg8bit.png', 0, len(item)):
                ...
                newImage = transform(torch.where(mytensor, img, sImg))
                img = transform(img)
                sMap = transform(sMap)
